...
struct FItemAddResult
{
	...
	FItemAddResult() : ActualAmountAdded(0), OperationResult(EItemAddResult::IAR_NoItemAdded),
	...
	static FItemAddResult AddedPartial(const int32 PartialAmmountAdded, const FText& ErrorText)
	{
	...
	static FItemAddResult AddedAll(const int32 AmountAdded, const FText& Message)
	{
		...
		AddedAllResult.OperationResult = EItemAddResult::IAR_PartialAmountItemAdded;
...
protected:
	...
	FItemAddResult HandleNoneStackableItem(UItemBase* ItemIn, int32 RequestedAddAmount);
	int32 HandleStackableItem(UItemBase* ItemIn, int32 RequestedAddAmount);
	int32 CalculateWeightAddMount(UItemBase* ItemIn, int32 RequestedAddAmount);
	int32 CalculateNumberForFullStack(UItemBase* StackableItem, int32 InitialRequestedAddAmount);

	//스택 가능 여부를 판단
	void AddNewItem(UItemBase* Item, const int32 AMountAdd);
public:	
	// Called every frame
